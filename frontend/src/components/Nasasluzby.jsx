const Nasasluzby = () => {
  return (
    <div
      id='ourServices'
      className=' font-oswald text-gray-700 mx-auto p-2 lg:mt-28 flex flex-col  '
    >
      {/* LEFT  */}
      <div className='mx-auto lg:mt-[200px]'>
        <div className='text-center w-fit mx-auto '>
          <h1 className='text-3xl lg:text-6xl  mx-auto text-white p-4 m-2  bg-gray-700/10 transition-all duration-300 ease-in-out hover:scale-105 backdrop-blur-sm font-robotoLight font-thin border-2'>
            <span className='text-color-green '>[</span>
            Naše služby
            <span className='text-color-green'>]</span>
          </h1>
        </div>

        <ul className='uppercase text-gray-200  flex-wrap flex text-lg text-center justify-center  font-robotoLight font-normal lg:font-thin  md:text-2xl md:mt-4 min-w-[300px]   max-w-[1200px]'>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50 p-3 m-1 h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            obkladanie do rozmeru 160x320cm
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Rekonštrukcie bytov domov
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Montáž sanity
          </li>

          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Murárske práce
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Sadrokartóny
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Pokladka dlažby
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Jolly hrany
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Vonkajšej, dlažby na terče
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Maľovanie
          </li>
          <li className='lg:text-2xl hover:scale-105 duration-300 bg-black/50   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            {' '}
            Realizacia krbov
          </li>
        </ul>
      </div>
      {/* Taktie vieme  */}
      <div className=' text-white mx-auto lg:mt-24 '>
        <h2 className='w-fit mt-28 text-3xl text-center lg:text-6xl  mx-auto text-white p-4 m-2  bg-gray-700/10 transition-all duration-300 ease-in-out hover:scale-105 backdrop-blur-sm font-robotoLight font-thin border-2'>
          {/* <span className='text-color-green text-4xl font-normal'>
                    |
                  </span> */}
          <span className='text-color-seablue '>[</span>
          Taktiez vieme
          <span className='text-color-seablue'>]</span>
        </h2>
        <ul className=' uppercase text-gray-200 flex-wrap flex text-lg text-center font-robotoLight font-normal lg:font-thin md:text-2xl md:mt-4 lg:text-2xl   plus:text-2xl max-w-[1200px]'>
          <li className='w-full lg:text-xl hover:scale-105 duration-300 bg-black/70   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            Zabezpezit dovoz materiálu{' '}
          </li>
          <li className='w-full lg:text-xl hover:scale-105 duration-300 bg-black/70    p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            Vybaviť vám 30-40% zľavy na daný material.{' '}
          </li>
          <li className='w-full lg:text-xl hover:scale-105 duration-300 bg-black/70   p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            Konzultacia ohladom vyberu obkladu, dlažby do domu/bytu.
          </li>
          <li className='w-full lg:text-xl hover:scale-105 duration-300 bg-black/70 p-3 m-1   h-auto text-sm md:text-lg lg:w-[200px] lg:h-[200px] lg:items-center lg:flex justify-center border-l-2 border-color-green shadow-2xl'>
            K dispozícii máme vodoinštalatera, elektrikára a iných remeselníkov.
          </li>
        </ul>
      </div>
    </div>
  )
}
export default Nasasluzby
